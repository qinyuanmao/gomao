...
func NewClickHouseDB(key string) (*db.MaoDB, error) {
	...
	dsn := fmt.Sprintf("tcp//%s?database=%s&username=%spassword=%s&read_timeout=%d&write_timeout=%d", address, dbName, username, password, address, readTimeout, writeTimeout)
