// fork from https://github.com/opentracing-contrib/go-stdlib/blob/master/nethttp/client.go
// get parent span context from metadata
package network
...
func (h *Tracer) start(req *http.Request) opentracing.Span {
	if h.root == nil {
		...
		if ok {
			...
			if sctx, err := opentracing.GlobalTracer().Extract(opentracing.TextMap, carrier); err == nil {
		...
		root := h.tr.StartSpan(operationName, opts...)
		h.root = root
	}
